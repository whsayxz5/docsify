# 聚收银简介

- [聚收银注册](#聚收银注册)
- [聚收银使用教程](#聚收银使用教程)
- [聚收银常见问题](#聚收银常见问题)
- [聚收银交易时间](#聚收银交易时间)
- [聚收银代理](agent/jsy.md)
- [其他说明](#其他说明)

---

# 聚收银注册

##### **一、免费领取**

[<img src="https://cos.zjkmkj.com/media/2024/08/20/e8f5d144a22c5dfd8f51e36fce1f4bc1-2.webp" style="zoom:33%;" />](http://u.zjkm.xyz/X5CJy)

##### 二、主要特点

**快捷方便**：无需机器，在线注册，完全免费

**聚合支付**：快付通/恒信通/拉卡拉等，多支付通道，可~~快捷~~/微信/支付宝/~~银联扫码~~

**笔笔有分：** 不在任何黑名单，全行有分

**注意事项：** 除 `内蒙古`外，全国可用，18-70岁均可注册

##### 三、快速选择

**快付通：** 每月可修改商户名

**恒信通：** 支付宝/微信/银联扫码`暂时下架`

**首信易：** 招行有积分，可修改商户名

**拉卡拉（新）：** 支付宝微信

**银盛：** 大额支付特殊通道

> 首信易需手动提现

##### 四、交易限额

| 支付方式（支付宝/微信） | 费率+手续费 | 单笔限额 | 单日限额 |
| ----------------------- | ----------- | -------- | -------- |
| 快付通（微信）          | 0.38%       | 1万元    | /        |
| 恒信通（支付宝/微信）   | 0.38%       | 2万元    | 10万元   |
| 拉卡拉（支付宝/微信）   | 0.38%       | 3万元    | 10万元   |
| 海科（支付宝）          | 0.55%+1     | 10万元   | 20万元   |
| 电银（支付宝）          | 0.55%+1     | 50万元   | 50万元   |
| 银盛（支付宝）          | 0.60%       | 10万元   | 50万元   |

| 支付方式（快捷/银联扫码） | 费率      | 单笔限额  | 单日限额   |
| ------------------------- | --------- | --------- | ---------- |
| ~~恒信通（银联扫码）~~    | ~~0.55%~~ | ~~5万元~~ | ~~10万元~~ |
| ~~快付通（快捷）~~        | ~~0.60%~~ | /         | /          |
| 首信易（快捷）   | 0.60%       | 1000元   | 1000元   |

> 机器注册当日无法秒到，次日正常可秒到

##### 五、购买费用

无需机器，在线注册，完全免费

##### 六、关于代理

[🔗 点此开通代理](agent/jsy.md)，注册分享即可推广，5户起共享封顶政策

---

# 聚收银使用教程

##### 一、领取码牌

##### [🔗 点我领取](http://u.zjkm.xyz/X5CJy)自助领取

<img src="https://cos.zjkmkj.com/media/2024/08/20/93ed4c477be9f80e86ed40b04583507b-2.webp" width=800 /><br>

##### 二、用户注册

`第一步领取的网址`，发送到任意微信聊天框，点击链接开始注册

<img src="https://cos.zjkmkj.com/media/2024/12/20/136e94928848fee0ac87fdef27d4b318-2.webp" width=800 /><br>

##### 三、开通商户

**（一）开通快付通**

> 广发银行限制餐饮类交易，关键字：餐、饮、烧烤、糕、馆、麻辣烫、吃、饭、米线、饺、菜、食、火锅、肉、汤、花甲、黄焖鸡、披萨、砂锅、饺、水果捞、奶茶、包子、早餐、鸡排等

<img src="https://cos.zjkmkj.com/media/2024/08/20/08363b833c8d9a69a21dcf0fd0cc3229-2.webp" width=800 /><br>

**（二）开通恒信通支付**

> 恒信通商户名称建议: xx市xx行业

<img src="https://cos.zjkmkj.com/media/2024/08/20/9d9c6fe4ece2d78fa817dbe6416edf5d-2.webp" width=800 /><br>

**（三）开通首信易**

> **[👉点我下载首信易钱包](https://webox.5upay.com/shouhuibao/index)**

<img src="https://cos.zjkmkj.com/media/2024/08/20/d7ba2dcf588ed47cdfd031fade580c47-2.webp" width=800 /><br>

**（四）开通拉卡拉**

<img src="https://cos.zjkmkj.com/media/2024/08/20/ff25df5211714a65df8a747dc124a4d0-2.webp" width=800 /><br>

**（五）开通银盛码**

> 行业类别建议优先选择批发、珠宝、数码、教育、建材类
> 
> 注意：门头环境照片不能乱拍其他不相关信息

<img src="https://cos.zjkmkj.com/media/2024/08/20/649a687575f68810ff2b3695001ce18d-2.webp" width=800 /><br>

**（六）开通电银通**

<img src="https://cos.zjkmkj.com/media/2025/01/02/c018eb2d313646a678ef696b054ff0ce-2.webp" width=800 /><br>

##### 四、扫码认证 `微信支付宝需要认证`

**开通支付宝：**按照[🔗 支付宝认证教程](tool/zfbrz.md)操作

**开通微信：**按照[🔗 微信认证教程](tool/wxrz.md)操作

##### 五、获取收款码 `重要`

👉`商户运营管理助手`小程序 -码牌-截图保存，保存收款码即可

<img src="https://cos.zjkmkj.com/media/2025/01/04/7a8b6cc918c4c5583ec37059f8d64ac0-2.webp" width=400 /><br>

##### 六、正常使用

**快捷支付：**微信扫码-扫收款码，选择特惠支付/标准支付，绑卡交易即可。

**银联扫码：**云闪付APP扫码-扫收款码，选择云闪付，按照提示付款即可。

**微信支付：**微信扫码-扫收款码，选择微信-选择通道商户，按照提示付款即可。

**支付宝支付：**支付宝扫码-扫收款码，选择支付宝-选择通道商户，按照提示付款即可。

<img src="https://cos.zjkmkj.com/media/2025/01/04/225c88f303fa35a9ba73fa0688c06936-2.webp" width=800 /><br>

##### 七、一键开通功能

快付通、恒信通：名称随机生成

> 其他渠道根据基本信息匹配，也可以点修改资料改名称

<img src="https://cos.zjkmkj.com/media/2024/08/20/eb74cc3acb84aea52948a4b6ad23b8cb-2.webp" width=800 /><br>

---

# 聚收银常见问题

##### 一、交易失败

用户已被发卡行风控，无法进行交易

##### 二、不支持此银行

太小众的银行不支持交易，建议换卡交易

##### 三、短信验证码不正确

客户需检查交易银行卡在银行预留的手机号码是否正确

##### 四、下单失败

交易时间段 `07:00~22:00`

##### 五、换结算卡路径

商户小程序-商户开通-点对应的渠道

> `恒信通、快付通`暂时不支持修改结算卡

##### 六、交易金额

交易金额不可为 100 的整数倍，如 `4600元/5900元/14200元`等；如提示 `支付失败（额度不足）`，建议降低交易金额重试

##### 七、使用距离

50KM

##### 八、首信易出现会员状态异常 `未激活，不可用`

此商户超限，已被注销

##### 九、快付通显示`无合适路由`

降低金额或者换卡重新试

##### 十、小程序无法打开

若无法打开小程序使用，可[👉点击下载](https://merchant.wnqianbao.com/index)`聚合运营伙伴`APP进行注册

---

# 聚收银交易时间

**银盛/恒信通/拉卡拉：** 全天24小时

**海科：** 早上00:00点—晚上22:30点

**电银：** 早上06:00点—晚上23:00点

**快付通：** 早上07:00点—晚上22:00点

> 机器注册当日无法秒到，次日正常可秒到

---

# 其他说明

一、以上介绍和说明，如有疑问或歧义的地方可随时 `联系我司客服`；如正常下单使用默认为已知晓并确认所有注意事项，避免因本文相关内容个人理解引起的误会。

二、以上产品信息和介绍仅供参考，具体 `以实际交易为准`。支付行业受国家相关监管和当地人行监管因素，`机器政策、功能、限额、费率`总部会根据市场政策进行增减和调整，任何 `支付公司总部`均保留调整上述功能/手续费的权利并会通过官方渠道进行通知，同时我司也会进行及时转达，以本页面实时资料为准；如您对调整后的功能/手续费存在异议，您应停止使用本产品并联系我司客服进行更换升级；若您仍继续使用的，则视为接受调整后的功能/手续费。

三、`扫码交易`属于附带功能，特别是支付宝微信，目前支付宝/微信全面严控，与机器/用户/玩法都无关，不存在无风控随便刷的路子，机器本身支持扫码不代表支付宝微信都能正常使用信用卡付款，如出现无法选择信用卡/支付风控/等问题属于正常行为，与机器本身刷卡功能无关，不作为机器使用问题之一。

四、原则上刷卡机属于特殊物品，寄出不退不换；我司承诺提供更高标准的 `七天无理由退换`，在 `机器未使用`和 `不影响二次销售`的前提下，用户可无理由在 `签收后七天内`进行退换，`来回运费自付`。

五、如有任何不明之处，请随时联系我司客服。

